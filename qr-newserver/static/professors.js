...
{
    const response = await fetch('/pk?name=docentes');
    const data = await response.json();
    return data;
}
...
<div class='col-sm-4'><h3>Gestion de <b>Docentes</b></h3></div>
...
<span style="vertical-align:middle;">Eliminar Docente(s)</span>
</button>
</div>
</div>
<div class="row">
...
let openModalCreate = function()
{
	...
	fields.forEach(field => {
	    ...
	    content[field] = $(`[id='create${field}']`).val();
	});
